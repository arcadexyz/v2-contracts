...
import "./specialty-interfaces/ISuperRare.sol";
 ...
 * 
 * THIS VAULT IS TO ONLY BE USED FOR STORING ```````SUPER RARE``````` NFTS
 *
 ...
 */
contract SuperRareVault is IAssetVault, OwnableERC721, Initializable, ERC1155Holder, ERC721Holder, ReentrancyGuard {
    ...
    // super rare address
    address public immutable superRareAddress = 0x41A322b28D0fF354040e2CbC676F0320d8c8850d;
    ...
    /**
     * @notice Withdraw entire balance of a given SuperRare token from the vault.
     ...
     * @param tokenId               The ID of the SuperRare to withdraw.
     * @param to                    The recipient of the withdrawn token.
     *
     */
    function withdrawSuperRare(
        address token,
        uint256 tokenId,
        address to
    ) external onlyOwner onlyWithdrawEnabled {
        require(token == superRareAddress, "AssetVault: Only SuperRare NFTs");

        ISuperRare(token).transfer(to, tokenId);
